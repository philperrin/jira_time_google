...
Opens a modal where the user fills in details. On submit, the Jira REST API is
called to create the issue and then the Assignments sheet is refreshed.
-----------------------------------------------
*/
function createJira() {
...
function getDropdownValues() {
  ...
  const createJiraProj = sheet.getRange('B2:B' + sheet.getLastRow()).getValues();
...
function makeJira(formData) {
  ...
  const API_KEY = userProperties.getProperty('JIRA_API_KEY');
  const USER_EMAIL = Session.getActiveUser().getEmail();
  const authHeader = 'Basic ' + Utilities.base64Encode(`${USER_EMAIL}:${API_KEY}`);
  ...
  const payload = {
    fields: {
      ...
      description: {
        ...
        content: [{
          type: 'paragraph',
          content: [{ type: 'text', text: formData.input5 || '' }]
        }]
      },
      issuetype: { name: 'Task' },
      customfield_10201: { value: formData.input2 },
...
function sendTime() {
  ...
  // Almost done - let's clear up the Time Card sheet, the Assignments sheet, and the Calendar sheet to prep for the next round of time logging.
  const TIMECARDSHEET = CURR_SPREADSHEET.getSheetByName('Time Card');
  TIMECARDSHEET.getRange("B2:D105").clearContent();
  
  const ASSIGNMENTSSHEET = CURR_SPREADSHEET.getSheetByName('Assignments');
  const namedRanges = ASSIGNMENTSSHEET.getNamedRanges();
        namedRanges.forEach(nr => nr.remove());

  const CALENDARSHEET = CURR_SPREADSHEET.getSheetByName('Calendar');
  const calRange = CALENDARSHEET.getRange('H2:H' + CALENDARSHEET.getLastRow());
        calRange.clearDataValidations();
  
  // Finally, let's calculate the date of the next Monday and populate it in cell A2 on the Time Card sheet. This way you can just jump right in and start logging your time without having to worry about setting up the dates for the week.
